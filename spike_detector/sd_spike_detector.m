function sd_spike_detector(path, filename, thr, t_before, t_after, max_t_on, min_t_off, toler, j_wind, bckg_med_wind, sp_ampl, aux)
...
if nargin < 2 
    [filename, path] = uigetfile('*.csv','Select .csv file with calcium activity traces', 'D:\VOVA\PLACE_CELLS\data\Test.csv');
end

if nargin < 11 || isempty(thr) || isempty(t_before) || isempty(t_after) || isempty(max_t_on) || isempty(min_t_off) || isempty(toler) || isempty(j_wind) || isempty(sp_ampl) || isempty(aux)
    ...
    bckg_med_wind = str2double(dlg_data{8});
    sp_ampl = dlg_data{9};
    aux = dlg_data{10};
end
