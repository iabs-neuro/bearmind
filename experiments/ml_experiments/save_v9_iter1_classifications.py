"""
Save expert classifications for v9_iter1 corrections.
"""
import pandas as pd

# Expert classifications from manual review
REAL_FP = [24, 29, 38, 45, 42, 51, 57, 58, 61, 65, 71, 79, 81, 82, 84, 88, 91, 94]
REAL_FN = [44, 47, 52, 58, 59, 62, 63, 67, 69, 72, 92, 97, 99]

print('='*80)
print('V9 ITERATION 1 - EXPERT CLASSIFICATIONS')
print('='*80)
...
df_spatial = pd.read_csv('ml/ebm_v9_iter1/fp_spatial_analysis.csv')
...
import json
with open('ml/ebm_v9_iter1/expert_classifications.json', 'w') as f:
...
print(f'\nSaved to: ml/ebm_v9_iter1/expert_classifications.json')

print('\nNext step:')
print('  python ml/apply_corrections.py \\')
print('    --dataset ml/results/training_dataset_v9.csv \\')
print('    --fp-errors ml/ebm_v9_iter1/top100_fp.csv \\')
print('    --fn-errors ml/ebm_v9_iter1/top100_fn.csv \\')
print(f'    --real-fp {",".join(map(str, sorted(REAL_FP)))} \\')
print(f'    --real-fn {",".join(map(str, sorted(REAL_FN)))} \\')
print('    --fp-spatial ml/ebm_v9_iter1/fp_spatial_analysis.csv \\')
print('    --output ml/results/training_dataset_v9_corrected_iter1.csv')
