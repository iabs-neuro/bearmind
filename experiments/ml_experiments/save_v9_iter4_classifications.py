"""
Save expert classifications for v9_iter4 corrections.
"""
import pandas as pd
import json

# Expert classifications from manual review
REAL_FP = [6,7,10,13,14,16,17,18,19,20,22,23,26,27,28,32,39,40,46,47,59,60,65,75,87,88,89,91,93,94,99]
REAL_FN = [1,2,6,8,9,36,37,41,56,66,76,94]

print('='*80)
print('V9 ITERATION 4 - EXPERT CLASSIFICATIONS')
print('='*80)
...
df_spatial = pd.read_csv('ml/ebm_v9_iter4/fp_spatial_analysis.csv')
...
with open('ml/ebm_v9_iter4/expert_classifications.json', 'w') as f:
...
print(f'\nSaved to: ml/ebm_v9_iter4/expert_classifications.json')
...
print(f'  Iter 4: {len(REAL_FP) + len(REAL_FN)} ({len(REAL_FP)} FP + {len(REAL_FN)} FN)')

print('\nInterpretation:')
if len(REAL_FP) + len(REAL_FN) > 35:
    print('  Real errors INCREASED significantly.')
    print('  This suggests either:')
    print('    - Model is finding harder edge cases')
    print('    - Dataset has systematic labeling issues')
    print('    - Approaching limit of model capability')
else:
    print('  Real errors stable or decreasing.')
    print('  Continue iterating for further improvement.')

print('\nNext step:')
print('  python ml/apply_corrections.py \\')
print('    --dataset ml/results/training_dataset_v9_corrected_iter3.csv \\')
print('    --fp-errors ml/ebm_v9_iter4/top100_fp.csv \\')
print('    --fn-errors ml/ebm_v9_iter4/top100_fn.csv \\')
print(f'    --real-fp {",".join(map(str, sorted(REAL_FP)))} \\')
print(f'    --real-fn {",".join(map(str, sorted(REAL_FN)))} \\')
print('    --fp-spatial ml/ebm_v9_iter4/fp_spatial_analysis.csv \\')
print('    --output ml/results/training_dataset_v9_corrected_iter4.csv')
