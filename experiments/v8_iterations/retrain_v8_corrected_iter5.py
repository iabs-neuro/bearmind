"""
Retrain v8 model on corrected_iter5 dataset.
Uses same hyperparameters as original v8 but seed=45 (for comparison with iter4).
...
THRESHOLD = 0.75
RANDOM_SEED = 45  # Same as iter4 for fair comparison

print('='*80)
print('RETRAINING v8 ON CORRECTED_ITER5 DATASET (SEED=45)')
print('='*80)

# Load corrected_iter5 dataset
...
# Train new model (same hyperparameters, seed=45)
print(f'\n{"="*80}')
print(f'TRAINING v8_corrected_iter5 (seed={RANDOM_SEED})')
print('='*80)

v8_corrected_iter5 = ExplainableBoostingClassifier(
...
v8_corrected_iter5.fit(X_train, y_train)
...
y_train_proba = v8_corrected_iter5.predict_proba(X_train)[:, 1]
y_test_proba = v8_corrected_iter5.predict_proba(X_test)[:, 1]
...
# Save model
output_path = 'production_models/ebm_v8_corrected_iter5.pkl'
with open(output_path, 'wb') as f:
    pickle.dump(v8_corrected_iter5, f)
...
df_fp.to_csv('ml/results/v8_corrected_iter5_top100_fp.csv', index=False)
df_fn.to_csv('ml/results/v8_corrected_iter5_top100_fn.csv', index=False)

print(f'\nError reports saved:')
print(f'  ml/results/v8_corrected_iter5_top100_fp.csv ({len(df_fp)} neurons)')
print(f'  ml/results/v8_corrected_iter5_top100_fn.csv ({len(df_fn)} neurons)')

print(f'\n{"="*80}')
print('ITERATION 4 vs 5 COMPARISON (seed=45)')
print('='*80)

# Load iter4 results for comparison
print('\nIter4 results (seed=45): F-beta=0.9241, FP=745, FN=684, Total=1429')
print(f'Iter5 results (seed=45): F-beta={test_metrics["fbeta"]:.4f}, FP={n_fp}, FN={n_fn}, Total={n_fp+n_fn}')
print(f'\nImprovement: {((test_metrics["fbeta"]/0.9241 - 1)*100):+.2f}% F-beta, {n_fp+n_fn-1429:+d} errors')
