"""
Retrain v8 model on corrected_iter2 dataset.
Uses same hyperparameters as original v8 but seed=44.
...
THRESHOLD = 0.75
RANDOM_SEED = 44  # Changed from 43

print('='*80)
print('RETRAINING v8 ON CORRECTED_ITER2 DATASET (SEED=44)')
print('='*80)

# Load corrected_iter2 dataset
...
print(f'\n{"="*80}')
print(f'TRAINING v8_corrected_iter2 (seed={RANDOM_SEED})')
print('='*80)

v8_corrected_iter2 = ExplainableBoostingClassifier(
...
v8_corrected_iter2.fit(X_train, y_train)
...
y_train_proba = v8_corrected_iter2.predict_proba(X_train)[:, 1]
y_test_proba = v8_corrected_iter2.predict_proba(X_test)[:, 1]
...
# Save model
output_path = 'production_models/ebm_v8_corrected_iter2.pkl'
with open(output_path, 'wb') as f:
    pickle.dump(v8_corrected_iter2, f)
...
df_fp.to_csv('ml/results/v8_corrected_iter2_top100_fp.csv', index=False)
df_fn.to_csv('ml/results/v8_corrected_iter2_top100_fn.csv', index=False)

print(f'\nError reports saved:')
print(f'  ml/results/v8_corrected_iter2_top100_fp.csv ({len(df_fp)} neurons)')
print(f'  ml/results/v8_corrected_iter2_top100_fn.csv ({len(df_fn)} neurons)')

print(f'\n{"="*80}')
print('NEXT STEPS')
print('='*80)
print(f'\n1. Visualize errors:')
print(f'   python visualize_v8_corrected_iter2_errors.py')
...
print(f'\n3. Apply corrections and retrain (iteration 3)')
print(f'\n{"="*80}')
