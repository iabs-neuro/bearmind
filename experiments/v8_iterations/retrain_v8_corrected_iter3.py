"""
Retrain v8 model on corrected_iter3 dataset.
Uses same hyperparameters as original v8 but seed=45.
...
THRESHOLD = 0.75
RANDOM_SEED = 45  # Changed from 44

print('='*80)
print('RETRAINING v8 ON CORRECTED_ITER3 DATASET (SEED=45)')
print('='*80)

# Load corrected_iter3 dataset
...
print(f'\n{"="*80}')
print(f'TRAINING v8_corrected_iter3 (seed={RANDOM_SEED})')
print('='*80)

v8_corrected_iter3 = ExplainableBoostingClassifier(
...
v8_corrected_iter3.fit(X_train, y_train)
...
y_train_proba = v8_corrected_iter3.predict_proba(X_train)[:, 1]
y_test_proba = v8_corrected_iter3.predict_proba(X_test)[:, 1]
...
# Save model
output_path = 'production_models/ebm_v8_corrected_iter3.pkl'
with open(output_path, 'wb') as f:
    pickle.dump(v8_corrected_iter3, f)
...
df_fp.to_csv('ml/results/v8_corrected_iter3_top100_fp.csv', index=False)
df_fn.to_csv('ml/results/v8_corrected_iter3_top100_fn.csv', index=False)

print(f'\nError reports saved:')
print(f'  ml/results/v8_corrected_iter3_top100_fp.csv ({len(df_fp)} neurons)')
print(f'  ml/results/v8_corrected_iter3_top100_fn.csv ({len(df_fn)} neurons)')

print(f'\n{"="*80}')
print('NEXT STEPS')
print('='*80)
print(f'\n1. Visualize errors:')
print(f'   python visualize_v8_corrected_iter3_errors.py')
...
print(f'\n3. Apply corrections and retrain (iteration 4) if needed')
print(f'\n{"="*80}')
