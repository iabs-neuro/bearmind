...
FIXED: Load estimates from data/raw_compressed/ directory.
"""
import pickle
import numpy as np
import pandas as pd
...
def load_estimates_from_raw(session_name, raw_dir='data/raw_compressed'):
    ...
    # Try HDF5 format
    hdf5_files = list(raw_path.glob(f'{session_name}*.hdf5'))
    if hdf5_files:
        try:
            with h5py.File(hdf5_files[0], 'r') as f:
                # This would need proper HDF5 parsing
                pass
        except:
            pass
...
def visualize_errors(
    ...
    print('='*80)
    print('VISUALIZING v8 MODEL ERRORS (FIXED)')
    print('='*80)
    ...
    for page in range(n_fp_pages):
        ...
        for i, (idx, row) in enumerate(page_neurons.iterrows()):
            ...
            # Load estimates
            est = load_estimates_from_raw(session, raw_dir)
