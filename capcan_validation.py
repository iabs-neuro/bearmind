...
def validate_single_session(session_id, init_file, gt_file, fps=None, verbose=True, event_method='threshold', n_iter=2, hybrid_kinetics=False):
    ...
    metrics_df_init, match_mtx_init, FCD_init, FBD_init, _, _, _ = estimates_to_metrics(
        est_init,
        fps=fps,
    ...
    metrics_df_gt, _, _, _, _, _, _ = estimates_to_metrics(
        est_gt,
        fps=fps,
    ...
    metrics_df_auto_final, _, _, _, _, _, _ = estimates_to_metrics(
        est_auto,
        fps=fps,
...
def batch_validate(mapping, fps=None, include_heavy=True, output_file=None, save_artifacts=True,
    ...
    for session_id, (init_file, gt_file) in tqdm(mapping.items(), desc="Validating"):
        try:
            ...
            metrics_df_init, match_mtx_init, FCD_init, FBD_init, corner_info, _, _ = estimates_to_metrics(
                est_init,
                fps=session_fps,
                include_event_based=True,
